...
def uploadProduct():
    if request.method == "POST":
        ...
        modalnumber = request.form['modalnumber'] 
        ...
        ListOfFile = {'a':filename55,'b':idofproduct,'c':name,
                      'd':category,'e':Price,'f':user,'g':passwww,'h':quantity,'i':modalnumber}
        return ListOfFile 
...
def generateqrcode():
    if request.method == "POST":
        ...
        for i in modalno.split(','):          
            data = username+"|"+new_file1['Hash']+"|"+idofp+"|"+pname+"|"+pcat+"|"+pprice+"|"+str(i)+"|"+'static/qrcode/'+imagename.split('.', 1)[0]+"_"+str(i)+'_QR.png'  
            ...
            img.save('static/qrcode/'+imagename.split('.', 1)[0]+"_"+str(i)+'_QR.png') 
        
        ListOfFile = {'a':idofp,'b':pname,'c':pcat,'d':pprice,'e':imagename.split('.', 1)[0],'f':modalno}
        return ListOfFile 
    return render_template('displayProduct.html')
...
@app.route('/claimownership', methods=['GET', 'POST'])
def claimownership():
    if request.method == 'POST':
        print("GET")
        name = request.form.get("name") 
        number = request.form.get("number")
        str1 = request.form.get("list") 
        parts2 = str1.split("|")

        imgname = parts2[7]

        data = name+"|"+parts2[1]+"|"+parts2[2]+"|"+parts2[3]+"|"+parts2[4]+"|"+parts2[5]+"|"+parts2[6]+"|"+parts2[7]+"|"+number+"|Done"
        ...
        pathlib.Path('static/keys/unknow').mkdir(exist_ok=True)
        with open('static/keys/unknow/'+str(res)+'_private.pem', 'wb' ) as f:
            f.write( key.exportKey( 'PEM' ))        
        ...
        dataofqr = strencry+","+'static/keys/unknow/'+str(res)+'_private.pem'
